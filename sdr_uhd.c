...
static void *uhd_tx_thread_ep(void *arg) {
    ...
    while (!uhd_tx_thread_exit) {
         ...
         if (iq != NULL && iq->data16 != NULL) {
             ...
             size_t samps = iq->validLength / 2;
             ret = uhd_tx_streamer_send(
               uhd.tx_streamer, &(iq->data16), samps, &uhd.md, 0.1, &num_samps_sent);
             if (ret || (samps != num_samps_sent)) {
               gui_status_wprintw(RED, "Error pushing TX buffer: %d, requested %u sent %u\n", ret, samps, num_samps_sent);
               break;
             }
...
int sdr_uhd_init(simulator_t *simulator) {
    ...
    ret = uhd_usrp_make(&uhd.usrp, "type=b200");
    if (ret) {
    ...
    double freq_gps_hz = TX_FREQUENCY;
